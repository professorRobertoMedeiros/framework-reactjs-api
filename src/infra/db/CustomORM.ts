...
import { BaseModel, ENTITY_META_KEY, COLUMN_META_KEY, INDEX_META_KEY, BUSINESS_INDEX_META_KEY, ColumnOptions, IndexOptions, TIMESTAMPS_METADATA_KEY, SOFT_DELETE_METADATA_KEY } from '../../core/domain/models/BaseModel';
...
export class CustomORM {
  ...
  private generateCreateTableSQL(model: typeof BaseModel): string {
    ...
    // Adicionar colunas extras para timestamps e soft delete
    if (hasTimestamps) {
    ...
    if (hasSoftDelete) {
      columnDefinitions.push('deleted_at TIMESTAMP DEFAULT NULL');
    }
  ...
  private async addMissingColumns(model: typeof BaseModel): Promise<void> {
    const tableName = model.getTableName();
    ...
    try {
      ...
      for (const model of this.models) {
        ...
        try {
          ...
          if (hasSoftDelete) {
            console.log(`Modelo ${model.name} possui decorador @SoftDelete`);
          }
        } catch (error) {
          console.error(`Erro ao verificar decoradores para ${model.name}:`, error);
          console.error(`TIMESTAMPS_METADATA_KEY: ${String(TIMESTAMPS_METADATA_KEY)}, SOFT_DELETE_METADATA_KEY: ${String(SOFT_DELETE_METADATA_KEY)}`);
